...
export namespace Tracker {
  ...
  export function promise<T>(promise: Promise<T>, desc?: string, trackedFor?: unknown): Promise<T> {
    return defineProperty(promise, kTracked, Object.assign({
      parent: parentOf(promise),
      for: trackedFor,
      time: Date.now(),
      trace: getTrace(Tracker.promise),
      desc,
    } satisfies TrackInfo, createRaw(promise)))
  }


  // Tracked `Promise.any` alternative
  export function promiseAny<T extends readonly unknown[] | []>(promises: T, desc?: string, trackedFor?: unknown): Promise<T[number]> {
    return defineProperty(Promise.any(promises), kTracked, Object.assign({
      parent: parentOf(promises),
      for: trackedFor,
      time: Date.now(),
      trace: getTrace(Tracker.promiseAny),
      desc,
    } satisfies TrackInfo, createRaw(promises)))
  }

  export function callback<T, A extends unknown[]>(callback: (...args: A) => T, desc?: string): (...args: A) => T {
    return defineProperty(callback, kTracked, Object.assign({
      parent: parentOf(callback),
      for: callback.name,
      time: Date.now(),
      trace: getTrace(Tracker.callback),
      desc,
    } satisfies TrackInfo, createRaw(callback)))
  }
}
