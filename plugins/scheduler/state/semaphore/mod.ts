...
import { Callback, SchedulerState } from '../base.ts'
...
export class Semaphore extends SchedulerState {
  ...
  completancy<T>(callback: Callback<T>): Promisify<T> {
    const { promise: disposed, resolve: interrupt }
      = Promise.withResolvers<void>()
    this.ctx.on('dispose', interrupt)
    const { promise, resolve } = Promise.withResolvers<T>()
    return <Promisify<T>> this.next(
      callback.name,
      Tracker.promiseAny(
        [
          Tracker.promise(
            promise,
            'Semaphore::completancy@done',
            callback.name,
          ),
  ...
  period(tagOrPeriod?: string | number, maybePeriod?: number): Promise<void> {
    ...
    if (tag) {
      return this.next(
        tag,
        Tracker.promiseAny(
          [Tracker.promise(after(period), 'Semaphore::period@after'), disposed],
  ...
  nextSlow = async (promise: Promise<unknown>, tag?: string): Promise<void> => {
    ...
    return Tracker.promise(
      Promise.resolve(),
      'Semaphore::nextSlow@resolve',
      tag,
    )
  }

  next = (
    promiseOrTag: Promise<unknown> | string | undefined,
